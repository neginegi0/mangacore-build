 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`mangacore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`mangacore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'mangacore-' + name : 'mangacore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/mangacore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = browserifyPath + ' --require ./index.js:' + fullname + ' --external mangacore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = browserifyPath + ' --require ./index.js:mangacore-lib -o mangacore-lib.js';
    }
  ...
  gulp.task('release:push', function(cb) {
    git.push('mangacoinproject', 'master', {
      args: ''
    }, cb);
  });
  ...
  gulp.task('release:push-tag', function(cb) {
    ...
    git.tag(name, 'Release ' + name, function() {
      git.push('mangacoinproject', name, cb);
    });
  });
  ...
  var release = function(importance, cb) {
    ...
    return runsequence(
      ...
      // Run git push mangacoinproject $VERSION
      'release:push-tag',
      // Run npm publish
      'release:publish',
